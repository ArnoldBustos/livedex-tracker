...
const KANTO_REGIONAL_DEX_NUMBERS = Array.from(
    {
        length: 151
    },
    (_, index) => {
...
const HOENN_REGIONAL_DEX_NUMBERS = [
...
const getRegionalDexNumbers = (game: SupportedGame | null | undefined) => {
    if (game === "RUBY" || game === "SAPPHIRE" || game === "EMERALD") {
...
export const getDexEntriesForScope = ({
    ...
    const regionalDexNumbers = getRegionalDexNumbers(game);
