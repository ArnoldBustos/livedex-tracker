...
// ParsedSavePokemon stores the cross-generation fields needed to build a living dex snapshot.
// Gen 3 and Gen 4 parser modules map their decrypted party/box records into this shape.
...
// parser modules and upload flows use this so parser aggregation stays separate from dex response DTO shaping.
...
// parser modules expose this so uploads and dex services can reuse one parser-owned summary calculation.
...
// Gen-specific parsers use this so shiny and ownership counts stay parser-owned instead of being rebuilt downstream.
export const buildImportedDexSnapshot = ({
    ...
    partyPokemon: ParsedSavePokemon[];
    boxPokemon: ParsedSavePokemon[];
    ...
    const ownedPokemonByDexNumber = new Map<number, ParsedSavePokemon[]>();

    for (const pokemon of [...partyPokemon, ...boxPokemon]) {
        if (pokemon.speciesId <= 0 || pokemon.speciesId > maxNationalDexNumber) {
            continue;
        }
    ...
    for (let dexNumber = 1; dexNumber <= maxNationalDexNumber; dexNumber += 1) {
